...
TikTokで今バズっている**急上昇ハッシュタグ**(日本・アメリカ)を毎日自動更新で表示するサイトです。
自分のTikTok投稿のネタ探しと、アフィリエイト収益化を目的としています。

> **データの範囲について**
> - TikTok Creative Centerが未ログインで公開している範囲(各国 上位3件/過去7日間)を取得します
> - 楽曲ランキングはTikTok側のリニューアルで公開ページから廃止されたため現在は非対応です(復活すれば自動でサイトに表示されます)

## 仕組み

```
GitHub Actions(毎朝6時 JST)
  → scripts/fetch_trends.mjs がTikTok Creative Centerからデータ取得
    (認証付きAPIのため、ヘッドレスブラウザでページを開いて取得)
...
> **重要**: GitHubの無料プランでは、GitHub Pagesは**公開(Public)リポジトリでのみ**使えます。

1. **リポジトリを公開にする**: Settings → General → 最下部「Danger Zone」→ Change visibility → **Public**
2. **Pagesを有効にする**: Settings → Pages → 「Build and deployment」の Source を **GitHub Actions** にする
3. **Actions** タブ →「トレンドデータ毎日更新」→ **Run workflow** で一度実行
...
├── scripts/
│   └── fetch_trends.mjs           # データ取得スクリプト(Playwright使用)
└── .github/workflows/
